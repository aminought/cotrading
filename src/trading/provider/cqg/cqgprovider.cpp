...
std::unique_ptr<trading::model::Contract> CqgProvider::resolve_symbol(trading::model::Symbol symbol) {
    ...
    return std::make_unique<trading::model::Contract>(id, symbol_str);
}
...
std::vector<trading::model::TimeBar> CqgProvider::get_historical_data(
    ...
    auto complete = false;
    auto answer = this->client->send(client_msg.SerializeAsString());
    std::vector<trading::model::TimeBar> time_bars;
    while(!complete) {
        auto server_msg = WA::ServerMsg();
        server_msg.ParseFromString(answer);
        ...
        for(int i=0; i<report_size; ++i) {
            ...
            for(int j=0; j<bar_size; ++j) {
                ...
                auto time_bar = trading::model::TimeBar(time, open, close, high, low, volume);
