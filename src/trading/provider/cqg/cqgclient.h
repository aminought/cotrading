...
public:
    CqgClient() = default;
    CqgClient(const std::string& user_name, const std::string& password, const std::string& client_id, int account_id):
        user_name(user_name),
        password(password),
        client_id(client_id),
    ...
    virtual std::string get_next_answer(const std::chrono::system_clock::duration& timeout);
    ...
    void create_session(const std::string& token, const bpt::ptime& base_time);
private:
